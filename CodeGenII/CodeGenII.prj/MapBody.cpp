...
//  notePad << _T("  it = rslt.second ? rslt.first : myMap.end();") << nCrlf << nCrlf;
  ...
  notePad << _T("maxKey = rcd.") << tbl->keyName << _T(";") << nCrlf << nCrlf;
...
String          s;
  ...
  notePad << _T("iter.erase(); continue;}") << nCrlf << nCrlf;
...
String     fldName;
  ...
  for (fldDesc = iter(), i = 0; fldDesc; fldDesc = iter++, i++) {
    ...
    if (i == 0 && typ == WrtTyp)
            {notePad << _T("rcd.") << tbl->keyName << _T(" = dsc->value; continue;") << nCrlf; continue;}
