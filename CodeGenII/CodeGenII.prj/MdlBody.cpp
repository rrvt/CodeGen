...
void MdlBody::create() {
  ...
  notePad << _T("#include \"") << tblName << _T(".h\"") << nCrlf;
  notePad << _T("#include \"DAOfields.h\"") << nCrlf;
  notePad << _T("#include \"DAOrecords.h\"") << nCrlf;
...
String primeIndex = primaryKey->name;
  ...
  notePad << addTab(_T("DAOtable*"), tab) << _T("daoTbl = daoTables.bSearch(name);");
...
void MdlBody::addNewRcd() {
  notePad << rcdName << _T("* ") << tblName << _T("::add(");
  notePad << rcdName << _T("& rcd) {") << nCrlf;
  notePad << _T("DAOtable* daoTbl = daoTables.bSearch(name);   ");
  notePad << _T("if (!daoTbl) throw _T(\"Unable to locate ") << namePfx << _T(" Table\");") << nCrlf;
