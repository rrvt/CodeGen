...
String Address2;
  ...
  AddressRecord(const AddressRecord& r) : MapRecord(r), AddressID(r.AddressID), Address1(r.Address1),
           Address2(r.Address2) {}
 ~AddressRecord() {}

  AddressRecord operator= (AddressRecord& r) {
    copy(r, *this); AddressID = r.AddressID; Address1 = r.Address1; Address2 = r.Address2;
    }
...
public:
  ...
  AddressRecord* startLoop()  {it = myMap.begin();           return it != myMap.end() ? &it->second : 0;}
  AddressRecord* nextRecord() {return it == myMap.end() ? 0 :     ++it != myMap.end() ? &it->second : 0;}
  AddressRecord* startRLoop() {it = myMap.end(); return prevRecord();}
  AddressRecord* prevRecord() {return it == myMap.begin() ? 0 : &(--it)->second;}
  AddressRecord* curRcd()     {return it != myMap.end() ? &it->second : 0;}
