// Entity Map logic, Version 1.4.1
...
String Phone2;
  ...
  EntityRecord() : EntityID(0), FirstName(), MiddleInitial(), LastName(), Suffix(),
           ...
           Phone1(), Phone2() {}
  EntityRecord(const EntityRecord& r) : MapRecord(r), EntityID(r.EntityID), FirstName(r.FirstName),
           MiddleInitial(r.MiddleInitial), LastName(r.LastName), Suffix(r.Suffix), AddrID(r.AddrID),
           CityStID(r.CityStID), AddrIsPO(r.AddrIsPO), LocationZip(r.LocationZip), eMail(r.eMail),
           Phone1(r.Phone1), Phone2(r.Phone2) {}
 ~EntityRecord() {}

  EntityRecord operator= (EntityRecord& r) {
    copy(r, *this); EntityID = r.EntityID; FirstName = r.FirstName; MiddleInitial = r.MiddleInitial;
...
public:
  ...
  EntityRecord* startLoop()  {it = myMap.begin();           return it != myMap.end() ? &it->second : 0;}
  EntityRecord* nextRecord() {return it == myMap.end() ? 0 :     ++it != myMap.end() ? &it->second : 0;}
  EntityRecord* startRLoop() {it = myMap.end(); return prevRecord();}
  EntityRecord* prevRecord() {return it == myMap.begin() ? 0 : &(--it)->second;}
  EntityRecord* curRcd()     {return it != myMap.end() ? &it->second : 0;}
