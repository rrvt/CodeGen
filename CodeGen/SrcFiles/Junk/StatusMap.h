// Status Map logic, Version 1.4.1
...
String Description;
  ...
  StatusRecord(const StatusRecord& r) : MapRecord(r), StsID(r.StsID), Abbreviation(r.Abbreviation),
           Description(r.Description) {}
 ~StatusRecord() {}

  StatusRecord operator= (StatusRecord& r) {
    copy(r, *this); StsID = r.StsID; Abbreviation = r.Abbreviation; Description = r.Description;
    }
...
public:
  ...
  StatusRecord* startLoop()  {it = myMap.begin();           return it != myMap.end() ? &it->second : 0;}
  StatusRecord* nextRecord() {return it == myMap.end() ? 0 :     ++it != myMap.end() ? &it->second : 0;}
  StatusRecord* startRLoop() {it = myMap.end(); return prevRecord();}
  StatusRecord* prevRecord() {return it == myMap.begin() ? 0 : &(--it)->second;}
  StatusRecord* curRcd()     {return it != myMap.end() ? &it->second : 0;}
