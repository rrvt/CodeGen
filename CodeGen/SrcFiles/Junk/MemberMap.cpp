...
Rslt rslt;
  ...
  rslt = myMap.insert(make_pair(rcd.MemberID, rcd));

  it = rslt.second ? rslt.first : myMap.end();

  if (rslt.second && rcd.MemberID > maxKey) maxKey = rcd.MemberID;
...
MemberRecord* r;
  ...
  for (r = myTable.startLoop(); r; r = myTable.nextRecord()) {
    ...
    if (r->toDelete()) {erase(r->MemberID); myTable.erase(r->MemberID);}
...
bool        rslt;
  ...
  if (!rcdSet.findRecord(rcd.MemberID))
    {String s = _T("Unable to find AddressID index with ") + rcd.MemberID; return false;}

  rcdSet.edit();      // Required to update records with new values

  for (rslt = fields.startLoop(fld), i = 0; rslt; rslt = fields.nextField(fld), i++) {
    switch (i) {
      case  0 : if ((long) fld.value != rcd.MemberID) {
                  String s = _T("Update failed to find key: ") + rcd.MemberID;
...
bool MemberDB::add(MemberRecord& rcd) {
AceFieldDsc fld;
int         i;
variant_t   v;
bool        rslt;
  ...
  for (rslt = fields.startLoop(fld), i = 0; rslt; rslt = fields.nextField(fld), i++) {
    ...
    switch (i) {
      case  0 : rcd.MemberID = fld.value; continue;
      case  1 : v = rcd.BadgeNumber; break;
      case  2 : v = rcd.MbrEntityID; break;
      case  3 : v = rcd.EmplEntityID; break;
      case  4 : v = rcd.ICE_EntityID; break;
