...
String Txt;
  ...
  LocationPrefRecord(const LocationPrefRecord& r) : MapRecord(r), LocationPrefID(r.LocationPrefID), Key(r.Key),
           Txt(r.Txt) {}
 ~LocationPrefRecord() {}

  LocationPrefRecord operator= (LocationPrefRecord& r) {
    copy(r, *this); LocationPrefID = r.LocationPrefID; Key = r.Key; Txt = r.Txt;
    }
...
public:
  ...
  LocationPrefRecord* startLoop()  {it = myMap.begin();           return it != myMap.end() ? &it->second : 0;}
  LocationPrefRecord* nextRecord() {return it == myMap.end() ? 0 :     ++it != myMap.end() ? &it->second : 0;}
  ...
  LocationPrefRecord* curRcd()     {return it != myMap.end() ? &it->second : 0;}
