...
String Txt;
  ...
  AssgnPrefRecord(const AssgnPrefRecord& r) : MapRecord(r), AssgnPrefID(r.AssgnPrefID), APKey(r.APKey),
           Txt(r.Txt) {}
 ~AssgnPrefRecord() {}

  AssgnPrefRecord operator= (AssgnPrefRecord& r) {
    copy(r, *this); AssgnPrefID = r.AssgnPrefID; APKey = r.APKey; Txt = r.Txt;
    }
...
public:
  ...
  AssgnPrefRecord* startLoop()  {it = myMap.begin();           return it != myMap.end() ? &it->second : 0;}
  AssgnPrefRecord* nextRecord() {return it == myMap.end() ? 0 :     ++it != myMap.end() ? &it->second : 0;}
  AssgnPrefRecord* startRLoop() {it = myMap.end(); return prevRecord();}
  AssgnPrefRecord* prevRecord() {return it == myMap.begin() ? 0 : &(--it)->second;}
  AssgnPrefRecord* curRcd()     {return it != myMap.end() ? &it->second : 0;}
