...
BOOL CodeGenView::PreCreateWindow(CREATESTRUCT& cs) {return CScrView::PreCreateWindow(cs);}
...
DataSource ds = doc()->dataSrc();
  ...
  if (printing)
    switch(ds) {
      ...
      case DataBaseSrc : prtDsc.print(*this);  break;
      }

    else
      switch(ds) {
        case NotePadSrc  : dspNote.display(*this); break;
        case DataBaseSrc : dspDsc.display(*this);  break;
        }
...
void CodeGenView::printFooter(Device& dev, int pageNo) {
  switch (doc()->dataSrc()) {
    ...
    case DataBaseSrc : prtDsc.footer(dev, pageNo); break;
    }
  }
