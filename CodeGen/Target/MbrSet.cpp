...
void MbrSet::DoFieldExchange(CFieldExchange* pFX) {
  ...
  RFX_Text(pFX, _T("SecondaryEmail"),      secondaryEmail);
  RFX_Text(pFX, _T("TextMsgPh1"),          textMsgPh1);
  RFX_Text(pFX, _T("TextMsgPh2"),          textMsgPh2);
  RFX_Text(pFX, _T("HandHeld"),            handHeld);
  RFX_Text(pFX, _T("PortMobile"),          portMobile);
  RFX_Text(pFX, _T("PortPacket"),          portPacket);
  RFX_Text(pFX, _T("OtherEquip"),          otherEquip);
  RFX_Text(pFX, _T("Multilingual"),        multilingual);
  RFX_Text(pFX, _T("OtherCapabilities"),   otherCapabilities);
  RFX_Text(pFX, _T("Limitations"),         limitations);
  RFX_Text(pFX, _T("Comments"),            comments);
  RFX_Text(pFX, _T("ShirtSize"),           shirtSize);
  RFX_Bool(pFX, _T("IsOfficer"),           isOfficer);
  RFX_Text(pFX, _T("SkillCertifications"), skillCertifications);
  RFX_Text(pFX, _T("EOC_Certifications"),  eOC_Certifications);
...
#if 0
notePad << _T("noFields  = ") << GetODBCFieldCount() << nCrlf;
notePad << _T("noRecords = ") << GetRecordCount() << nCrlf;
notePad << _T("RowSize   = ") << GetRowsetSize() << nCrlf;
GetStatus(status);
notePad << _T("CurIndex  = ") << status.m_lCurrentRecord << nCrlf;
notePad << _T("FinalRcdCnt= ") << status.m_bRecordCountFinal << nCrlf;
      notePad << _T("RowStatus(50) = ") << GetRowStatus(50) << nCrlf;
      notePad << _T("RowStatus(51) = ") << GetRowStatus(51) << nCrlf;
      notePad << _T("RowStatus(52) = ") << GetRowStatus(52) << nCrlf;
